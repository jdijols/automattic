...
> **Product name is a TODO.** This repo lives in a folder named `Automattic` only because [Automattic](https://automattic.com) (parent of WordPress.com) is the evaluator for this Gauntlet AI Partner Project — Challenger submission. It is **not** an Automattic product. A real product name (candidate: **Blocksmith**) will be chosen before any live deployment. See the Naming section in `CLAUDE.md`.
...
4. **The product name is still the `Automattic` placeholder.** `package.json` is `wp-block-theme-generator`; the page renders "WordPress Block Theme Generator". A real name (candidate: Blocksmith) is deferred pending selection — renaming mid-flight also breaks tooling pinned to the current path.
5. **The supported-block allowlist is a deliberately closed set.** `contract/allowlist.json` enumerates the blocks the system will author (it excludes `core/html`, `core/shortcode`, `core/freeform`, and all third-party namespaces). Expansion is an explicit PR, never a runtime fallback — so a generator request that needs a block outside the set fails loudly rather than degrading into a free-tree escape hatch. Pattern-library breadth (and therefore design distinctiveness) is bounded accordingly; the breadth roadmap is in [`docs/phase-2-prd.md`](docs/phase-2-prd.md) and "What I'd Do Next" (pending).
