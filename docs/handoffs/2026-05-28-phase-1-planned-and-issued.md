...
- **gbrain — Night-1 gotcha now *diagnosed, not resolved*.** The `/sync-gbrain --full` run this session imported the docs as text pages (`default` source = 141 pages) but **every embedding failed**: gbrain's config has no embedding field so it defaults to OpenAI, and neither `OPENAI_API_KEY` nor `VOYAGE_API_KEY` is in the environment — so no vectors are generated and semantic `gbrain search` is non-functional (capability check fails). **Fix:** export an embedding key (or configure gbrain for a local no-key model) before launching `claude`, then re-run `/sync-gbrain --full`. Separately, gbrain is **v0.18.2** — too old for the code-index stage (`reindex-code` arrived in v0.20.0+), so the code source has 0 pages; code search needs a gbrain upgrade. **Until embeddings work, use Grep, not `gbrain search`.** Repo-trust policy may want setting now that `origin` exists.
