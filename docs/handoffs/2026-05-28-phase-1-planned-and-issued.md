...
- **gbrain:** a remote now exists, and embeddings were backfilled via `/sync-gbrain --full` this session — the Night-1 "empty embeddings" gotcha is resolved. The repo-trust policy may want setting now that `origin` exists.
